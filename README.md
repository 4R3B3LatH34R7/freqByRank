...
So, I'd be a little bit cautious not to use this UDF on cell ranges.

### 3.2.Rank - optional - default=1
The UDF could be called minimally as =freqByRank(A1:F16) and it will return the value(s) within the said range which has the highest occurences(frequencies).\
Above is true for uni-modal datasets but for multi-modal data sets, it should be called as an array formula with Ctrl+Alt+Enter resulting in something like {=freqByRank(A1:F16)}.\
So the question is how do we know if the dataset is uni/multi-modal?\
Simple, we don't!, untill we actually call the UDF.\
On Office365, which has spill feature, there is no need for Ctrl+Alt+Enter as entering normally like =freqByRank(A1:F16) alone is enough and it will return an array to the right of the formula cell if there are more than one cell with the same frequency, automagically.\
However, on older Excel versions, users can still select a number of columns to the RIGHT of the formula cell and enter like =freqByRank(A1:F16,2) and enter as an array formula to see how many non-#N/A values are returned to check how many cells to select to be included into the array. The number 2 in this example asks the UDF to return the value(s) with the second highest frequency.\
...
1 was actually set as the default rank so that calling =freqByRank(A1:F16) will only return the value(s) with maximum occurences within the dataset/range.\
...
If the requested Rank value is higher than the available/possible rank values, the UDF shall return a 0. For example, if the result contains only 4 ranges of frequencies like 1, 2, 3 & 4 and if the user asked for a rank 6, like =freqByRank(A1:F16,6), it will return a 0.\

If the user wishes to have the UDF return frequencies for all the cells, the UDF could be called as =freqByRank(A1:F16,0) as an ARRAY formula but in this example 96 rows in the column containing the formula cell because A1:F16 hold 16x6=96.\
...
In the prior scenario, the third parameter, returnCount comes to the rescue by returning only the count instead of that 49-ish column single-row array by calling the UDF as =freqByRank(A1:F16,4,TRUE).\
<b>NB: To prevent an array of numbers if the UDF were called with returnCount=TRUE, the UDF is now limited to return the count only if it is not called as an array formula.</b>

### 3.4.return1D - optional - default=TRUE
The actual switch/argument for calling UDF as =freqByRank(A1:F16,0) and get the result in a single column, is the 4th argument and it could be turned on/off as =freqByRank(A1:F16,0,,TRUE/FALSE).\
If left out like =freqByRank(A1:F16,0) or =freqByRank(A1:F16,0,,) will yield the same result as return1D was set to TRUE by default.\
...
In such cases where the unique values are most abundant, we can ask the UDF not to return the unique values with a switch like =freqByRank(A1:F16,0,,,TRUE) with a TRUE for the fifth argument and this is for include1s and set to FALSE by default.\
